...
const serverFrontEnd = app.listen(port, '127.0.0.1', () => {
...
serverFrontEnd.on('upgrade', (request, socket, head) => {
    if (request.url === '/ws1') {
...
app.get('/tolocalws', (req, res) => {
    ...
    if (connectStaues == "open") {
        console.log("1yes")
        const ws = new WebSocket(`ws://${connectDeviceIp}:${connectDevicePort}/ws-local`); // 与对方建立一个连接实例
