...
const wsef = ws.WebSocket('ws://127.0.0.1:33451') //建立前后端的连接实例

// 本机对外的node ws接口 在ip：端口
ws.on('connection', (ws) => {

    ws.send(JSON.stringify({ status: 'connected', devicename: uniqueDeviceId })); //连接成功返回状态与唯一 会话uuid
    ...
    ws.addEventListener('message', (event) => {
        ...
        if (connectdevice != lastcconnectdevice) {
            ...
            while (1) {
                wsef.send(JSON.stringify({ localreqlinknames: reqlinknames, status: 'accept' }));
            }
        }
    ...
    if (connectStatus == "open") {
        ...
        fromws.addEventListener('message', (event) => {
            ...
            if (jsonwsdata.status == "connected") {
                ...
                res.status(200).json({ LinkStatus: "connectok", uniqueDeviceId: jsonwsdata.uniqueDeviceId }); // 发给我的前端验证 发送二者会话id
        ...
        fromws.addEventListener('message', (event) => {
            ...
            res.status(200).json({ LinkStatus: "closeok", uniqueDeviceId: jsonwsdata.uniqueDeviceId });// 发送给前端 确认关闭
        });
