...
@app.route("/api/nodeserve",methods=["GET"])
def nodeserve():
    global node_process
    isopen_node = request.args.get('isopen')
    if isopen_node == '1':

        try:
            if node_process:
                return jsonify({"message": "服务已经在运行","status_serve_node":'1-1'}), 400
            
            node_process = subprocess.Popen(['node', '../wscf/wsReceiver.js'])  # 启动一个新的 Node.js 进程
            if node_process:
                return jsonify({"message": "服务启动成功","status_serve_node":'1-0'})
            else:
                return jsonify({"message": "服务启动失败请检查配置","status_serve_node":'1-2'})

        except Exception as e:
            logging.warning(e)
    elif isopen_node == '0':
            if not node_process:
                return jsonify({"message": "服务尚未启动","status_serve_node":'2-1'}), 400

            # 终止正在运行的 Node.js 进程
            os.kill(node_process.pid, signal.SIGINT)  # 发送 SIGINT 信号终止进程
            node_process = None  # 清空进程引用
            return jsonify({"message": "服务停止成功","status_serve_node":'2-0'})
...
def serv():
    ...
    if isopen == '1' and not IFSTART:
        IFSTART = True
        th_start = threading.Thread(target=zeroconfLocal.zeroconfins.start_service, args=(info,))
        th_start.start()
        th_start.join()
        return jsonify({
    ...
    # 关闭服务
        ...
        IFSTART = False
        th_stop = threading.Thread(target=zeroconfLocal.zeroconfins.stop_service, args=(info,))
        th_stop.start()
        th_stop.join()
        return jsonify({
