        ...
        for iface, addrs in psutil.net_if_addrs().items():
            ...
            for addr in addrs:
                ...
                if addr.family == socket.AF_INET6:
                    if has_ipv4:  # 如果当前接口已经有 IPv4 地址
                        temp_addr += addr.address.split("%")[0]
...
def serv():
    ...
    info = zeroconfLocal.ServiceInfo(
        ...
        prot = 4 if ipv4 else 6
        sa =f"socket.AF_INET{prot}"
        print(sa)
        addresses=[socket.inet_pton(sa,local_ip)],  # 本地 IP 地址
