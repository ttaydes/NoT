...
class MyListener:
    ...
    def add_service(self, zeroconf, type, name):
        ...
        dev_info = {"device_name":dev_name,"device_ip":ip_address}

        if dev_info not in self.local_online_device and dev_name!='_' + socket.gethostname().split('.')[0]:
    ...
    def remove_service(self, zeroconf, type, name):
        ...
        info = zeroconf.get_service_info(type, name)
        device_names = [device['device_name'] for device in listener.local_online_device]
        dev_name = name.split('.')[0]
        ...
        if dev_name in device_names:
            ...
            print(f"删除过之后的{self.local_online_device}" )
